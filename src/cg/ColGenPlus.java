...
public class ColGenPlus extends ColGen<SNDRC, Cycle, SNDRCPricingProblem>{
	...
	public ColGenPlus(SNDRC dataModel, 
			...
			int cutoffValue,
		  	double boundOnMasterObjective,double gapTolerance) {
	...
	public void solve(long timeLimit) throws TimeLimitExceededException{
		...
		do{
			...
			//after the update of objectiveMasterProblem £¬we check if stop column generation process
			...
			//Case2: objectiveMasterProblem is smaller than cutoff value(here we should note infeasible case,set the objective value of artificial variables big enough compared to cutoff value)
			if(objectiveMasterProblem<cutoffValue-config.PRECISION&& gap1<0.3) {
			...
			//Case3:objectiveMasterProblem is over cutoff value and  boundOnMasterObjective below cutoff value
			if(objectiveMasterProblem>cutoffValue+config.PRECISION&&!boundOnMasterExceedsCutoffValue()) {
				if(gap0>0&&gap0-gap1<gapTolerance-config.PRECISION&&gap1<0.3) {
			...
			if(System.currentTimeMillis() >= timeLimit){ //Check whether we are still within the timeLimit
