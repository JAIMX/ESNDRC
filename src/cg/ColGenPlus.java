...
public class ColGenPlus extends ColGen<SNDRC, Cycle, SNDRCPricingProblem>{
	...
	public void solve(long timeLimit) throws TimeLimitExceededException{
		...
		do{
			...
			//after the update of objectiveMasterProblem and possibly the bound on the master problem objective, we check if stop column generation process
			...
			//Case2: objectiveMasterProblem is smaller than cutoff value
			if(objectiveMasterProblem<cutoffValue-config.PRECISION) {
			...
			if(objectiveMasterProblem>cutoffValue+config.PRECISION&&!boundOnMasterExceedsCutoffValue()) {
				if(gap0>0&&gap0-gap1<gapTolerance-config.PRECISION) {
